...
#        if level < 1 or level > 50:
    ...
    def solve(self):
        soko.solve_map("levels.txt", self.lev, "A*", "", "")
        self.solved_matrix = []
        f = open("temp.txt", "r")
        for line in f:
            for c in line:
                if (c == 'r' or c == 'l' or c == 'u' or c == 'd'):
                    self.solved_matrix.append(c)
        f.close()
       

    def solved_move(self, move_index):
        time.sleep(0.1)
        if (self.solved_matrix[move_index] == 'l'):
            game.move(-1, 0, True)
        elif (self.solved_matrix[move_index] == 'r'):
            game.move(1, 0, True)
        elif (self.solved_matrix[move_index] == 'u'):
            game.move(0, -1, True)
        elif (self.solved_matrix[move_index] == 'd'):
            game.move(0, 1, True)
        time.sleep(0.1)
...
reset = False
while 1:
    if (game.is_completed() and not reset): display_end(screen)
    print_game(game.get_matrix(),screen)
    if (game_solved == 1 and move_index < len(game.solved_matrix)):
        game.solved_move(move_index)
